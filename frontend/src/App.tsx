...
import type { Artist, ProductItem, CartItem } from './types';
...
const AppContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch Artists
        const artistRes = await fetch('http://127.0.0.1:8001/api/artists/');
        if (!artistRes.ok) throw new Error('Failed to fetch artists');
        const artistData = await artistRes.json();
        setArtists(artistData.artists);

        // Fetch Merch Items
        const merchRes = await fetch('http://127.0.0.1:8001/api/merch/');
        ...
        interface MockItem { id: number; artistId: number; name: string; price: number; condition: 'new' | 'secondhand'; type: string; size: string | null; stock: number; desc: string; }
        ...
        const mockItems: ProductItem[] = allItems.map(p => {
          const matchedArtist = mockArtists.find(a => a.id === p.artistId) || mockArtists[0];
          return {
            id: p.id,
            ...
            sticker_color: artistColorMap[matchedArtist.slug] || 'yellow',
          };
        });
  ...
  const onViewProduct = (product: ProductItem) => {
